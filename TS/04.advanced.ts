...
/*
    3.Generics
     - Type을 변수처럼 외부에서 주입받아 사용하는 기능으로
       타입변수라고 불린다.
     - Type에 유연성을 제공하면서 확장성과 타입 안전성을 추가할 수 있다.
     *Generics의 주요기능
...
//TypeScript는 구조중심의 타입검사 시스템을 사용하기 때문에
//다음과 같이 작성가능
...
function config<T extends Options = {theme:"dark"}>(option?:T):T{
    ...
    //널병합연산자 ??
    // - a ?? b -> a가 null 또는 undefiend인 경우 B반환, 아닌경우 A
    return option ?? {theme:"dark"} as T;
}
// 4) keyOf연산자와 Generic
...
const person:Person2 = {name:'mkm',age:16,location:'seoul'}
...
/*
     4.
      ...
      1) Partial<T>
       ...
       - 모든 속성을 필수, 읽기전욕으로 만들어주는 Required<T>,Readonly<T>도 존재한다.
*/
interface User{
    id:number;
    name: string;
...
//const user:User = {id:1,name:'aaa'};
const PartialUser:Partial<User> = {id:1,name:'aaa'};

/*
    2)Pick<T,K>
     - 객체의 속성 중 일부만 선택하여 새로운 타입을 만들 떄 사용
*/
type SimpleUser = Pick<User,"name"|"email">

// 3) Omit<T,K>
// - 객체타입에서 특정 속성을 제외하고 나머지만 남긴 새 타입을 만들 때 사용
// id를 제외한 사용자 정보
...
/*
    Utility Type 조합
    1. User 에서 id를 제외한 나머지 속성을 선택 후 전체를 옵셔널로 만들기
