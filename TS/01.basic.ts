/*
    1.타입주석
     ...
     - 타입주석을 통해 정적 타입 검사가 가능해지며, 잘못된 타입의 값이 들어갈 겨웅 컴파일 오류가 발생
*/
let num:number;
num = 12345;
//num = '12345';
...
/*
    2.타입추론
     ...
     - 단, 복잡한 함수의 매개변수나 반환형,api함수와 같이 명환한 타입정의가 필요한 곳에는 반드시 작성한다.
*/
let userName = 'kh 타이거';
let age=5;
...
/*
    ...
    - 일반배열은 언제든 요소의 수가 변할 수 있지만, 튜플은 항상 고정된 구조를 가장
*/
...
/*
    4. readonly
     ...
     - 타입스크립트의 정적타입체크 시스템은 실제 코드를 "실행 하기 전"단계에서 작동하므로, 실행환경에서
     동작하는 "메서드에 대한 검사"는 수행하지 않는다.
...
/*
    5.any Type
     - 어떤 값이든 저장할 수 있는 타입
     - 타입스크립트의 정적 타입검사를 우회하기위해 설계되었다.
     - javascript코드와의 호환성유지 및 마이그레이션을 돕기 위한 타입이고, 무분별하게 사용시 typescript의 사용의미가 사라진다.
*/
let any:any;
any = undefined;
any =1;
...
//단순히 값을 받기위한 용도로만 사용하며, unknown에 할당된 데이터를 재할당 하기 위해서는 적절한 타입으로의 narrowing필요
//str = unknown;
...
//as const
// - 객체나 배열에 저장된 값을 리터럴 타입으로 고정시켜 타입 안전성을 확보할 수 있게 도와주는 기능
