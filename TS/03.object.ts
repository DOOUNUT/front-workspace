...
const person1 = {name:'mkm',age:18};
const person2 = {name:'mkm',age:11};

/*
    2. interface
     - 객체의 구조를 정의하는 또 다른 방법
     - 인터페이스와 타입별칭의 동작방식은 유사하나 각기 존재하는 기능이 다르다
*/
interface Animal{
    name: string;
    kind: string;
...
const cat2:Cat = {name:'coco',kind:'cat',food:'사료'}
...
//a&b
const pizza = {name:'파파존스',flavor:'짠맛'};
// a
const bugger = {name:'맥도날드'}
//b
...
/*
    4. 결론
     ...
     1)interface가 적합한 경우
      - 클래스와 함꼐 사용시, 구조 확장과 유지보수가 필요한 경우
      - 디버깅시 타입표시가 필요한 경우
      - type별칭을 쓰든 interface를 쓰든 상관없는 경우
     2)
      - 유니온,교차타입등 복합 타입정의가 필요한 경우
...
//인덱스 시그니처 사용시 주의점
// - 속성에 여러 값들이 들어가는 경우, 인덱스 시그니쳐는 모든 값들에 대해 표현할 수 있어야한다.
const responseData2 = {
    name : 'kh academy' ,
    classRoom : 'C CLASS',
    teacher : 'mkm',
...
type ClassInfo = {
    ...
    studentCount2: number
}
/*
    6.타입호환성
     - 타입스크립트에서 두 타입이 서로 대입 가능한지를 판단하는 개념
     - 타입검사 매커니즘상 호횐되지 안흔 경우 컴파일 에러를 발생시킨다.
     - 타입 스크립트는 호환성 여부를 체크할 떄 "구조적 타이핑"에 기반하여
       호환 가능 여부를 판단한다.
     - 구조란 객체의 "속성명"과 그"타입"을 의미
     - 구조적 타이핑의 매커니즘으로 인해 객체의 모든 속성이 완전히 일치하지 않더라고 타입 호환이 가능하다.
*/
type Teacher = {
    name: string,
    age : number
