...
*/
function fnOptional(a:number, b?:number){
    console.log(b);
        /*
            b는 선택적 파라미터이므로 타입스크리는 b에 값이 올수도 있고 안 올수도 있으므로
            type | undefiend로 추론한다.
...
objectDesFn2({a:1,b:true,c:"mkm"})

/*
    7. 타입 내로잉
     - union type처럼 변수가 여러타입을 갖고 있을 때, 이를 사용하려고하는 한가지 타입으로 좁히는 문법
     - typeof, instenceof, in 연산자
*/
...
/*
    8.type Assertion
     - 여러 타입을 가질 수 있는 변수에 대하여 개발자가 해당 값의 타입을 명확히 알고 있는 경우 사용하는 문법
...
function typeNarrowing2(sOrn:etc):void{
    if(typeof sOrn === 'number'){
        ...
        // 현재 sOrn에 새로운 타입이 추가되는 경우 위 코드에서 컴파을 에러를 발생
        // 사람으로써 모든 타입이 철저히 검사되고 있는지 확인 할 수 있다.

        //타입체크용 메서드
        //컴파일 단계의 타입체크기능을 무효화하여 check 에 string,number와 같은 다른값이
        //전달된 경우, 런타임시 에러가 발생할 수 있도록 처리하는 함수
...
/* 
    10. 함수 타입 표현식/ 함수 시그니처
     ...
     - 함수의 시그너처? 함수가 가지는 매개변수의 개수와 자료형, 반환형을 합친 것.
*/
//일반함수
