class Person {
  constructor(public name:string,private age:number, private readonly addreses:string){
  }
  interoduction(){
    console.log(`안녕 난 ${this.name}이라고해.${this.addreses}에 살고 나이는 ${this.age}야`)
  }
}
const mkm3:Person = new Person('mkm',1234,'서울'); 
mkm3.introduction(); // 안녕 난 mkm이라고해.서울에 살고 나이는 1234야

class Pet{
    constructor(public kind:string,name:string){
...
class Hamster extends Pet{
    ...
    info() {
        ...
        this.food && console.log(`주식은 ${this.food}입니다`)
    }
}
