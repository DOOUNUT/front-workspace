...
function BoardList({list, setList}: {list: typeof boardList, setList: React.Dispatch<React.SetStateAction<typeof boardList>>}) {
  ...
  const sortByDateAsc = () => {
     const sortedDateAsc = [...list].sort((a,b) => a.boardNo - b.boardNo)
    setList(sortedDateAsc);
  } ;
  const sortByDateDesc = () => {
    const sortedByDateDesc = [...list].sort((a,b) => b.boardNo - a.boardNo)
    setList(sortedByDateDesc);
  };
      ...
      <h2>실습문제 2 : 게시판 목록</h2>
        ...
        <table className='list-table'>
          ...
          <tbody>
            ...
            //   })
              list.map((boardinfo)=>(
                 <tr key={boardinfo.boardNo}>{boardinfo.boardNo}
                     <th>{boardinfo.boardNo}</th>
                     <th>{boardinfo.boardTitle}</th>
                     <th>{boardinfo.boardWriter}</th>
                     <th>{boardinfo.boardDate}</th>
                     <th><button onClick={() => boardDelete(boardinfo.boardNo)}
                     style={{backgroundColor:'red',color:'white'}}>삭제</button></th>
                 </tr>
              ))
             }
          </tbody>
