...
const MenuEdit = () => {
    ...
    // const [newEdit,handleInputChange] = useInput<EditMenu>(editMenu);
    // const navigate = useNavigate();


    // const quertClient = useQueryClient();
    // const mutation = useMutation({
    //     mutationFn : (newEdit:EditMenu) => axios.put("http://localhost:8081/api/menus",newEdit),
    //     onSuccess : (res) =>{
    //         quertClient.invalidateQueries({queryKey:['menus']})
    ...
    // const editMenu = (e:FormEvent) =>{
    //     e.preventDefault();
    //     if(newEdit.restaurant ==''|| newEdit.name == ''){
    //         alert('모든필드 입력')
    //         return;
    //     }
    //     if(newEdit.price<0){
